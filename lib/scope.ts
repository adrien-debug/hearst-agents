...
export async function resolveScope(
  ...
  // 1. Resolve userId (auth required)
  ...
  // - Request headers (x-tenant-id, x-workspace-id)
  // - User's default tenant/workspace from DB
  // - Path params for tenant-scoped routes

  // For now, check env for explicit multi-tenant mode
  ...
  // 3. Dev fallback (explicit only)
  if (!tenantId || !workspaceId) {
    // Only allow fallback if not explicitly required
...
/**
 * Resolve scope or throw HTTP error response.
