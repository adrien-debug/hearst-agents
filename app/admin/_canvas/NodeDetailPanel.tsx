...
/**
 * Fiche technique d'un stage — toujours rendu dans la colonne droite.
 * L'empty state est à l'intérieur du bloc, pas un conditional qui masque le bloc.
 */
    ...
    return (
      <div className="flex flex-col gap-(--space-4) px-(--space-4) py-(--space-5) min-h-0 flex-1 justify-center">
        <div className="flex flex-col gap-(--space-1)">
          <p className="t-10 font-mono uppercase tracking-(--tracking-label) text-(--text-faint)">
            Stage du pipeline
          </p>
    ...
    <div className="flex flex-col min-h-0 flex-1">
      <header className="shrink-0 flex items-start justify-between gap-(--space-3) px-(--space-4) py-(--space-4) border-b border-line bg-surface">
        <div className="flex flex-col gap-(--space-1) min-w-0">
          ...
          <span className="t-10 font-mono uppercase tracking-(--tracking-label) text-(--text-faint)">
            {node.sublabel}
          </span>
        </div>
        <button
          type="button"
          onClick={onClear}
          className="t-10 font-mono uppercase tracking-(--tracking-caption) text-(--text-faint) hover:text-text transition-colors duration-(--duration-base) ease-(--ease-standard) shrink-0"
        >
          effacer
        </button>
      </header>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 flex flex-col gap-(--space-4) px-(--space-4) py-(--space-4)">
        <div className="rounded-(--radius-md) bg-bg-soft px-(--space-3) py-(--space-3) border border-line">
          <p className="t-10 font-mono uppercase tracking-(--tracking-caption) text-(--text-faint) mb-(--space-2)">
            Rôle
          </p>
        ...
        <div className="flex flex-col gap-(--space-2)">
          ...
          <div className="rounded-(--radius-sm) bg-bg-soft px-(--space-3) py-(--space-2) border border-line">
            <p className="t-11 font-mono tracking-(--tracking-hairline) text-(--cykan) break-all leading-snug">
              {node.fileHint}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-(--space-2)">
          ...
          <ul className="flex flex-col gap-(--space-2)">
            {node.events.map((e) => (
              <li
                key={e}
                className="t-11 font-mono tracking-(--tracking-hairline) text-text-soft pl-(--space-3) border-l-2 border-(--cykan)/25"
              >
                {e}
              </li>
            ))}
        ...
        <div className="pt-(--space-3) mt-auto border-t border-line">
          <span
            className={
              node.toggleable
                ? "t-9 font-mono uppercase tracking-(--tracking-stretch) text-(--cykan)"
                : "t-9 font-mono uppercase tracking-(--tracking-stretch) text-(--text-faint)"
            }
          >
