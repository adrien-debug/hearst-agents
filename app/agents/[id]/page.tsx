        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-950/80 p-5">
            <div className="mb-2 flex items-center justify-between">
              ...
              <Link href={`/runs?agent_id=${id}`} className="text-[10px] text-zinc-600 hover:text-zinc-400">
                Tout voir →
              </Link>
            </div>
            {runs.length === 0 ? (
              ...
              <div className="space-y-1">
                {runs.slice(0, 5).map((r) => (
                  <Link key={r.id} href={`/runs/${r.id}`} className="flex items-center justify-between rounded-md px-2 py-1 text-xs transition-colors hover:bg-zinc-900">
