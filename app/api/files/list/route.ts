...
import { driveConnector } from "@/lib/connectors";
...
export async function GET() {
  ...
  try {
    const result = await driveConnector.getFiles(userId, 15);
    ...
    if (message.includes("has not been used in project") || message.includes("is disabled")) {
      return NextResponse.json(
        { error: "api_not_enabled", message: "L'API Google Drive n'est pas activée. Activez-la dans la console Google Cloud." },
        { status: 503 },
      );
    }

    return NextResponse.json(
      { error: "drive_error", message: "Erreur de lecture Google Drive." },
      { status: 502 },
    );
  }
}
