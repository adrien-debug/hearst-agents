    ...
    return (
      <div className="flex h-full flex-col overflow-hidden">
        <div className="border-b border-zinc-800/60 px-6 py-5">
          ...
          <p className="mt-1 text-sm text-white/50">Connectez votre compte pour voir vos documents</p>
        </div>
        <div className="flex flex-1 flex-col items-center justify-center px-6">
          ...
          <p className="mt-5 text-sm text-white/70">Aucun stockage connecté</p>
      ...
      <div className="flex h-full flex-col overflow-hidden">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-white/70">Récupération de vos fichiers...</span>
          </div>
    ...
    return (
      <div className="flex h-full flex-col overflow-hidden">
        <div className="border-b border-zinc-800/60 px-6 py-5">
          ...
          <p className="mt-1 text-sm text-white/50">Aucun fichier récent</p>
        </div>
        <div className="flex flex-1 flex-col items-center justify-center px-6">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-zinc-800/40">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="h-8 w-8 text-white/50">
          ...
          <p className="mt-5 text-sm text-white/70">Aucun fichier récent</p>
        </div>
      </div>
    );
  }
          ...
          <div className="space-y-2">
            {files.map((file) => (
              <a
                ...
                className="flex items-center gap-3 rounded-lg border border-zinc-800/50 bg-zinc-900/40 px-4 py-3 transition-colors duration-200 hover:bg-zinc-800/50"
              >
                <span className="text-lg">{fileIcon(file.mimeType)}</span>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-white">{file.name}</p>
                  <div className="mt-0.5 flex items-center gap-2">
                    {file.size && <span className="text-[10px] text-white/50">{file.size}</span>}
                    {file.size && <span className="text-[10px] text-white/50">·</span>}
                    <span className="text-[10px] text-white/50">{formatRelativeTime(file.modifiedTime)}</span>
                  </div>
                </div>
                {file.shared && (
                  <span className="flex items-center gap-1 rounded-full bg-zinc-800/60 px-2 py-0.5 text-[10px] text-white/50">
                    Partagé
                  </span>
                )}
              </a>
