    ...
    return (
      <div className="flex h-full flex-col overflow-hidden">
        <div className="border-b border-zinc-800/60 px-6 py-5">
          ...
          <p className="mt-1 text-sm text-zinc-500">Connectez votre compte pour voir vos événements</p>
        </div>
        <div className="flex flex-1 flex-col items-center justify-center px-6">
          ...
          <p className="mt-5 text-sm text-zinc-400">Aucun calendrier connecté</p>
      ...
      <div className="flex h-full flex-col overflow-hidden">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-zinc-400">Récupération de vos événements...</span>
          </div>
    ...
    return (
      <div className="flex h-full flex-col overflow-hidden">
        ...
        <div className="flex flex-1 flex-col items-center justify-center px-6">
          ...
          <p className="mt-5 text-sm text-zinc-400">Votre agenda est libre cette semaine</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto">
        {Object.entries(grouped).map(([date, evts]) => (
          <div key={date} className="border-b border-zinc-800/40 px-6 py-4">
            <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-zinc-500">{date}</h2>
            <div className="space-y-2">
              {evts.map((ev) => (
                <div key={ev.id} className="flex items-center gap-3 rounded-lg border border-zinc-800/50 bg-zinc-900/40 px-4 py-3 transition-all duration-200 hover:bg-zinc-800/50">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {ev.location && <p className="mt-0.5 truncate text-[10px] text-zinc-600">{ev.location}</p>}
                  </div>
                </div>
              ))}
            </div>
