...
export function CapabilityBlockedBanner({
  ...
  if (!isExpanded) {
    return (
      <div className="flex items-center justify-between px-4 py-3 border-t border-b border-[var(--warn)] bg-[var(--bg-soft)]">
        <span className="t-10 font-mono uppercase tracking-[0.15em] text-[var(--warn)] flex items-center gap-2">
        ...
        <button
          ...
          className="t-9 font-mono uppercase tracking-[0.2em] text-[var(--text-muted)] hover:text-[var(--text)]"
        >
          Expand
        </button>
      </div>
  ...
  return (
    <div className="border-t border-[var(--warn)] bg-[var(--bg)]">
      <div className="flex items-start justify-between p-4 border-b border-[var(--line)]">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="t-10 font-mono uppercase tracking-[0.15em] text-[var(--warn)] mb-1">Capability Locked</p>
            <h3 className="t-13 font-black uppercase tracking-tighter text-[var(--text)]">Refus: {label}</h3>
            <p className="t-11 font-light text-[var(--text-muted)] mt-1">Connecter une source pour déverrouiller.</p>
          </div>
        </div>
        <button
          type="button"
      ...
      {draftMessage && onRestoreDraft && (
        <div className="px-4 py-3 border-b border-[var(--line)] flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0 flex-1">
            <span className="t-9 font-mono uppercase tracking-[0.1em] text-white/30">Draft</span>
            <p className="t-11 font-mono text-[var(--text-muted)] truncate mt-1">{draftMessage}</p>
          </div>
          <button
            type="button"
            onClick={() => onRestoreDraft(draftMessage)}
            className="font-mono t-9 uppercase tracking-[0.15em] text-[var(--cykan)] border-b border-[var(--cykan)] pb-0.5 flex items-center gap-1 shrink-0"
          >
      ...
      <div className="p-4">
        <p className="t-10 font-mono uppercase tracking-[0.15em] text-white/30 mb-4">Source Recognition</p>
          ...
          {missingServices.slice(0, 2).map((service) => (
            <div key={service.id} className="flex flex-wrap items-center gap-4 py-4 first:pt-0">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-[var(--text)] truncate">{service.name}</p>
                <p className="t-10 font-mono text-[var(--text-muted)] truncate">{service.description}</p>
              </div>
              <button
                type="button"
                onClick={() => handleConnect(service.id)}
                disabled={connectingId === service.id}
                className="ghost-btn-solid ghost-btn-cykan rounded-sm t-9 disabled:opacity-40"
              >
        ...
        <div className="mt-4 pt-4 border-t border-[var(--line)] flex items-center justify-between">
          <p className="t-9 font-mono uppercase tracking-[0.15em] text-[var(--text-faint)]">
            POOL_{missingServices.length}_SVC
          </p>
          <span className="t-9 font-mono uppercase tracking-[0.15em] text-[var(--cykan)] border-b border-[var(--cykan)] pb-0.5">
            Hub_Apps
          </span>
        </div>
      </div>
      ...
      {draftMessage && (
        <div className="px-4 py-3 border-t border-[var(--line)] bg-[var(--bg-soft)]">
          <p className="t-10 font-mono text-[var(--text-muted)] leading-relaxed">
...
export function InlineCapabilityBlock({ capability, onConnect }: InlineCapabilityBlockProps) {
  return (
    <div className="my-3 p-3 border-l-2 border-[var(--warn)] bg-[var(--bg-soft)]">
      <p className="t-10 font-mono uppercase tracking-[0.12em] text-[var(--warn)] mb-2">CAP_{capability}</p>
      <button type="button" onClick={onConnect} className="ghost-btn-solid ghost-btn-cykan rounded-sm t-9">
        Connect_Source
      </button>
    </div>
  );
